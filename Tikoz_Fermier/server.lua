...
ESX.RegisterServerCallback('Tikoz:Inventairefermier', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("Tikoz:CoffreDeposefermier")
AddEventHandler("Tikoz:CoffreDeposefermier", function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('Tikoz:CoffreSocietyfermier', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_fermier', function(inventory)
		cb(inventory.items)
	end)
end)

RegisterNetEvent('Tikoz:RetireCoffrefermier')
AddEventHandler('Tikoz:RetireCoffrefermier', function(itemName, count, itemLabel)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('Tikoz:fermierSalaire', function(source, cb)

    local xPlayer = ESX.GetPlayerFromId(source)
    local salairefermier = {}
...
RegisterServerEvent("Tikoz:fermierNouveauSalaire")
AddEventHandler("Tikoz:fermierNouveauSalaire", function(id, label, amount)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('Tikoz:fermierArgentEntreprise', function(source, cb)

    local xPlayer = ESX.GetPlayerFromId(source)
    local compteentreprise = {}


    MySQL.Async.fetchAll('SELECT * FROM addon_account_data WHERE account_name = "society_fermier"', {
...
RegisterServerEvent("Tikoz:fermierdepotentreprise")
AddEventHandler("Tikoz:fermierdepotentreprise", function(money)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local total = money
    ...
    TriggerEvent('esx_addonaccount:getSharedAccount', "society_fermier", function (account)
        if xMoney >= total then
            ...
            TriggerClientEvent('esx:showAdvancedNotification', source, 'Banque Société', "~b~fermier", "Vous avez déposé ~g~"..total.." $~s~ dans votre ~b~entreprise", 'CHAR_BANK_FLEECA', 9)
        else
...
RegisterServerEvent("Tikoz:fermierRetraitEntreprise")
AddEventHandler("Tikoz:fermierRetraitEntreprise", function(money)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local total = money
	local xMoney = xPlayer.getAccount("bank").money
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', "society_fermier", function (account)
		if account.money >= total then
			...
			TriggerClientEvent('esx:showAdvancedNotification', source, 'Banque Société', "~b~fermier", "Vous avez retiré ~g~"..total.." $~s~ de votre ~b~entreprise", 'CHAR_BANK_FLEECA', 9)
		else
			TriggerClientEvent('esx:showAdvancedNotification', source, 'Banque Société', "~b~fermier", "Vous avez pas assez d'argent dans votre ~b~entreprise", 'CHAR_BANK_FLEECA', 9)
		end
	end)
end) 
